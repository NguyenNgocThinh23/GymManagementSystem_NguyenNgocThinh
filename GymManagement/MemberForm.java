...
public class MemberForm extends JDialog {
    ...
    public MemberForm(Frame parent, Member m) {
        ...
        add(new JLabel("Gender:")); cbGender = new JComboBox<>(new String[]{"Male", "Female"}); add(cbGender);
        add(new JLabel("Phone:")); txtPhone = new JTextField(); add(txtPhone);
        add(new JLabel("Register Date:")); txtRegisterDate = new JTextField("dd/MM/yyyy"); add(txtRegisterDate);
        add(new JLabel("Package:")); cbPackage = new JComboBox<>(new String[]{"1 month", "3 months", "6 months", "12 months"}); add(cbPackage);
        add(new JLabel("Start Date:")); txtStartDate = new JTextField("dd/MM/yyyy"); add(txtStartDate);
        add(new JLabel("Trainer:")); txtTrainer = new JTextField(); add(txtTrainer);
        add(new JLabel("Status:")); cbStatus = new JComboBox<>(new String[]{"Active", "Expired", "Paused"}); add(cbStatus);

        JButton btnOk = new JButton("OK"), btnCancel = new JButton("Cancel");
        btnOk.addActionListener(e -> { isOk = true; dispose(); });
        btnCancel.addActionListener(e -> dispose());
        add(btnOk); add(btnCancel);

        if (m != null) {
            txtId.setText(m.getId()); txtId.setEditable(false);
